    ...
    for year in range(1995, 2022):
        for part in range(1, 3):
            urllib.request.urlretrieve(
                f'http://prod.publicdata.landregistry.gov.uk.s3-website-eu-west-1.amazonaws.com/pp-{str(year)}-part{str(part)}.csv', f'drive/MyDrive/dataset/pp-{str(year)}-part{str(part)}.csv')
            print(f'Downloaded {str(year)} part {str(part)}')


def upload_data(start_year, end_year):
    pass
